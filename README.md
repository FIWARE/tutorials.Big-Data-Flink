...
This command will also import seed data
...
Next, [download](https://github.com/ging/fiware-cosmos-orion-flink-connector/releases/latest) the connector JAR from the connector repository and run:
...
Once a dynamic context system is up and running, we need to inform **Flink** of changes in context.
...
case class Sensor(devide: String, sum: Int)
...
Now you can group the created objects by the type of device and perform operations on them:
...
-   **Message**: ```"urn:ngsi-ld:Lamp"+ node.id.takeRight(3)+ "@on"```
